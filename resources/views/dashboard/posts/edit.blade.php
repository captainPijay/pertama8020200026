  ...
  <div class="col-lg-8">
      ...
      <form method="POST" action="/dashboard/posts/{{ $post->slug }}" class="mb-5" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <input type="text" readonly class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" required value="{{ old('title',$post->slug) }}">
          @error('slug')
                <p class="text-danger">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="mb-3">
          <label for="body" v class="form-label">Body</label>
          @error('body')
             <p class="text-danger">{{ $message }}</p>
          @enderror
